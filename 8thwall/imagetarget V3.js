const imageTargetPipelineModule = () => {
  ...
  const sceneModels = [
    require('./assets/Z1.glb'),
    require('./assets/Z2.glb'),
    require('./assets/Z3.glb'),
    require('./assets/Z4.glb'),
  ]
  ...
  const videoAspectRatios = [1, 1, 1, 1]
  ...
  const initXrScene = ({scene, camera, renderer}) => {
    ...
    // Video Element and Video Texture
      ...
      videoMaterial[index] = new THREE.MeshBasicMaterial()
      videoMaterial[index].map = videoTexture[index]  // Set the new texture
      videoMaterial[index].needsUpdate = true
    }
    ...
    function switchIndex() {
      ...
      console.log(mixerAction[currentIndex])
      mixer[currentIndex].setTime(0)
      mixerAction[currentIndex].reset()
      mixerAction[currentIndex].play()
    }
    ...
    function resolve(gltf, index) {
      ...
      gltf.animations.forEach((clip) => {
        mixerAction[index] = mixer[index].clipAction(clip)
        mixerAction[index].play()
        mixerAction[index].setLoop(THREE.LoopOnce)
        mixerAction[index].clampWhenFinished = true  // Clamp the animation
      })
    }

    for (let i = 0; i < sceneModels.length; i++) {
      loader.load(sceneModels[i],
        (gltf) => {
          resolve(gltf, i)
        })
    }
  ...
  const showTarget = ({detail}) => {
    ...
    if (detail.name === 'protean-target-test') {
      const newScale = detail.scale * modelScale
      ...
      model[currentIndex].scale.set(newScale, newScale, newScale)
